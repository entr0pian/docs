...
This same tracking mechanism — not `ownerReferences` alone, since those can't cross a namespace boundary and an `Application`'s managed resources routinely live outside its own namespace — is also what makes deleting an `Application` able to cascade to everything it manages, gated behind the opt-in `resources-finalizer.argocd.argoproj.io` finalizer. `deletion.md` has the full trace, contrasted against the same-namespace, no-finalizer, `ownerReferences`-only case (a Deployment cascading to its ReplicaSet and Pods).
